...
const defaultCardImages = [
  ...
  "https://images.unsplash.com/photo-1533750516457-a7f992034fec?auto=format&fit=crop&q=80&w=800&h=500",
  "https://images.unsplash.com/photo-1504868584819-f8e8b4b6d7e3?auto=format&fit=crop&q=80&w=800&h=500",
  "https://images.unsplash.com/photo-1611162617474-5b21e879e113?auto=format&fit=crop&q=80&w=800&h=500",
];

// --- Helper function to generate financial-themed code ---
const generateFinancialCode = (width: number, height: number): string => {
  const dataPoints = [
    "REVENUE_LEAK_DETECTED...", "CALCULATING_PROJECTIONS...", 
    "CURRENT_MRR:$14,500", "ATTRITION: 2.4%", "LEAD_VELOCITY:+18%", 
    "AOV:$2,450", "CAC:$340", "LTV:$9,800", "CLOSE_RATE: 12%",
    "FLOWSTRA_AI_SYNCING...", "QUALIFYING_PIPELINE...", 
    "CONVERSION_OPTIMIZED", "TARGET_ROI: 3.2X", "STATUS: ACTIVE"
  ];
  const ASCII_CHARS = "0123456789{}[]<>;:._-=+*|/\\";
  let out = "";
  for (let i = 0; i < height; i++) {
    let line = "";
    while (line.length < width) {
      if (Math.random() > 0.8) {
        const word = dataPoints[Math.floor(Math.random() * dataPoints.length)];
        line += word + " ";
      } else {
        const chunkLength = Math.floor(Math.random() * 8) + 2;
        for (let j = 0; j < chunkLength && line.length < width; j++) {
...
const ScannerCardStream = ({
  ...
  const cards = useMemo(() => {
    ...
    return Array.from({ length: totalCards }, (_, i) => ({
      ...
      ascii: generateFinancialCode(Math.floor(400 / 6.5), Math.floor(250 / 13)),
    }))
  }, [cardImages, repeat]);
  ...
  useEffect(() => {
    ...
    const runScrambleEffect = (element: HTMLElement, cardId: number) => {
        ...
        const interval = setInterval(() => {
            element.textContent = generateFinancialCode(Math.floor(400 / 6.5), Math.floor(250 / 13));
            scrambleCount++;
  ...
  return (
    <main className="relative w-full h-[500px] flex items-center justify-center overflow-hidden">
      ...
      <canvas ref={particleCanvasRef} className="absolute top-1/2 left-[50%] -translate-x-[50%] -translate-y-1/2 w-full max-w-[100vw] h-[250px] z-0 pointer-events-none" />
      <canvas ref={scannerCanvasRef} className="absolute top-1/2 left-[50%] -translate-x-[50%] -translate-y-1/2 w-full max-w-[100vw] h-[300px] z-10 pointer-events-none" />
      ...
      <div className="absolute w-full h-[250px] flex items-center left-[50%] -translate-x-[50%]">
          ...
          {cards.map(card => (
            <div key={card.id} className="card-wrapper relative w-[400px] h-[250px] shrink-0">
              <div className="card-normal card absolute top-0 left-0 w-full h-full rounded-[15px] overflow-hidden bg-transparent shadow-[0_15px_40px_rgba(0,0,0,0.4)] z-[2] [clip-path:inset(0_0_0_var(--clip-right,0%))]">
                <img src={card.image} alt="Card" crossOrigin="anonymous" className="w-full h-full object-cover rounded-[15px] transition-all duration-300 ease-in-out brightness-110 contrast-110 shadow-[inset_0_0_20px_rgba(0,0,0,0.1)] hover:brightness-125 hover:contrast-125" />
              </div>
