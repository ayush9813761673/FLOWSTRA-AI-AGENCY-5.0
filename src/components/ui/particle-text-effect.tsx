...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  const animationRef = useRef<number>(undefined as any)
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full h-[500px] bg-transparent p-4 relative z-10 overflow-visible cursor-crosshair">
      <canvas
        ref={canvasRef}
        className="w-full h-full object-contain pointer-events-auto"
        style={{ maxWidth: "100%", height: "100%" }}
      />
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-white text-sm text-center max-w-md pointer-events-none opacity-50">
        <p className="text-xs">
          Right-click and hold to destroy particles
        </p>
      </div>
    </div>
  )
