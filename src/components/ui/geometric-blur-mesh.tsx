...
void transformVertices(out vec3 vertices[12], int count, mat3 rotation, float scale) {
    for (int i = 0; i < 12; i++) {
        if (i < count) {
...
export function GeometricBlurMesh() {
    ...
    return (
        <div ref={containerRef} className="relative w-full h-full bg-black cursor-pointer overflow-hidden rounded-xl">
            <canvas 
                ref={canvasRef}
                className="w-full h-full block"
            />
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 pointer-events-none text-center">
                <div className="text-white/30 text-[10px] font-medium tracking-widest uppercase">
                    {shapes[currentShape]}
                </div>
                <div className="text-white/15 text-[8px] font-light mt-0.5 tracking-wider uppercase">
                    Click pattern to cycle
                </div>
            </div>
        </div>
    );
